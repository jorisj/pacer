...
export class KmphToPaceComponent {
  speedKmh = signal(10); // Default speed
  commonSpeeds = [10, 11, 12, 13, 14, 15, 16, 20]; // Common running speeds

  paceMinKm = computed(() => this.calculatePace(this.speedKmh()));
  ...
  calculatePace(speed: number): string {
  ...
  calculateRaceTime(distanceKm: number): string {
